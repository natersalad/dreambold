...
# Add this variable to track the enabled state
...
func inputManagement():
	# Check if the player is enabled before processing inputs
	if not isEnabled:
		return
...
# Function to enable the player
func enable():
	isEnabled = true

# Function to disable the player
func disable():
	isEnabled = false
